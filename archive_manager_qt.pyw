...
#Checks for the file containing the directories of the pre-procssed files and the marks root
try:
    with open(path.join(dir,'dirs.txt'),'r') as f:
        filePos = f.read()
except IOError:
    print "File indicating directories not found."
    sys.exit()
finally:
    filePos = open('dirs.txt','r')
    dictPos = {'base':filePos.readline()[:-1],'root':filePos.readline()}
    filePos.close()   
...
try: #Checks for the file containing the list of all the marks and their associated directory
    ...
    markDict = mark.build_dict(dictPos['root'])
    for i in markDict:
...
class TransferThread(Thread): #Thread made switch on/off the file transfer
    ...
    def run(self):
        ...
        while 1:
            if Activated:
                try:
                    moveFeedback = transfer.move_file(dictPos['base'],markDict)
